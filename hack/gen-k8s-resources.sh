...
readonly UMBRELLA_CHART="${K8S_DEPLOY_DIR}/charts/kubeopsai"
...
main() {
  ...
  controller-gen rbac:roleName=k8s-engine-role-placeholder \
    crd paths="$REPO_ROOT_DIR/..." \
...
#  echo "Replace cluster role name k8s-engine-role with placeholder"
#  if [[ "$OSTYPE" == "darwin"* ]]; then
#    sed -i '' 's/k8s-engine-role-placeholder/k8s-engine-role-{{.Release.Namespace}}/g' "${K8S_DEPLOY_DIR}/charts/kubeopsai/charts/engine/templates/role.yaml"
#  else
#    sed -i 's/k8s-engine-role-placeholder/k8s-engine-role-{{.Release.Namespace}}/g' "${K8S_DEPLOY_DIR}/charts/kubeopsai/charts/engine/templates/role.yaml"
#  fi
