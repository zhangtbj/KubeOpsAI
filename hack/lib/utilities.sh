...
host::install::kubebuilder() {
  ...
  curl -L -o kubebuilder "https://go.kubebuilder.io/dl/v${KUBEBUILDER_VERSION}//${name}"
  chmod +x kubebuilder && mv kubebuilder /usr/local/bin/
  export GOPATH=${GOPATH:-$(go env GOPATH)}
  echo "GOPATH=$GOPATH"
  export PATH="$GOPATH/bin:$PATH"
  go install sigs.k8s.io/controller-runtime/tools/setup-envtest@latest
  KUBEBUILDER_ASSETS="$(setup-envtest use 1.22.1 -p path)"
...
capact::install() {
    ...
    export BUILD_IMAGES_LIST=${BUILD_IMAGES_LIST:-"argo-actions,argo-runner,e2e-test,gateway,hub-js,k8s-engine,portal-backend,populator,overlay-monitor,roles-service"}
